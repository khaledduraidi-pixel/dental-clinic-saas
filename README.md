...
من الإعدادات ← استيراد المرضى، يمكن رفع ملف **CSV** يحتوي أسماء وأرقام هواتف
مرضى العيادة القدامى دفعة واحدة، بدل إدخالهم يدوياً واحداً تلو الآخر. إذا
كانت البيانات في ملف إكسل، يُحفظ أولاً بصيغة CSV (ملف ← حفظ باسم ← CSV) ثم
يُرفع. الاستيراد:
...
اُستخدم مُحلِّل CSV مكتوب يدوياً بدل مكتبة مثل `xlsx` (SheetJS) عن قصد —
النسخة المتوفرة منها على npm تحمل ثغرات أمنية معروفة بدون إصلاح متاح
(Prototype Pollution وReDoS)، وهذه الميزة تحديداً تُشغَّل على ملفات يرفعها
المستخدم، أي بيانات أقل ثقة بطبيعتها.
